...
# Create DenseNet, DiceLoss and Adam optimizer.
...
for epoch in range(epochs):
    ...
    for batch_data in train_loader:
        ...
        # print(outputs.shape)
        # print(labels.shape)
        # outputs = outputs.view(8, 1, 1, 1)
        # outputs = F.interpolate(outputs, size=( 96, 96), mode='bilinear', align_corners=False)
...
export_prefix = daten + '_highresnet'
...
best_model_name = r'C:\Users\Y2LabServer4\downloads\ipynb_checkpoints\ipynb_checkpoints\20231017_best_metric_model.pth'
checkpoint_tmp = torch.load(r'\Users\Y2LabServer4\downloads\ipynb_checkpoints\ipynb_checkpoints\20231017_best_metric_model.pth')
post_trans = Compose([EnsureType(data_type='tensor'), AsDiscrete(threshold=6)]) 
...
with torch.no_grad():
    for test_data in test_loader:
        ...
        # test_outputs = outputs.view(8, 1, 96, 96)
        # test_outputs = test_outputs.view(1, 1, 1)
        ...
        #  print(f"Output: {test_outputs}")
        # print(f"Output all ones: {(test_outputs == 1).all()}")
        
        # print((test_outputs == 0).all())
